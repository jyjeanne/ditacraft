...
exports.detectDitaVersion = detectDitaVersion;
/**
 ...
 * 3. Default to '1.3'
 */
function detectDitaVersion(text) {
    // 1. Check DITAArchVersion attribute
    const versionAttrMatch = /DITAArchVersion\s*=\s*["']([^"']+)["']/.exec(text);
    if (versionAttrMatch) {
    ...
    if (doctypeMatch) {
        ...
        if (publicId.includes('1.0'))
            return '1.0';
        if (publicId.includes('1.1'))
            return '1.1';
        if (publicId.includes('1.2'))
            return '1.2';
        if (publicId.includes('1.3'))
            return '1.3';
        if (publicId.includes('2.0'))
            return '2.0';
    }
    // 3. Default
    return '1.3';
}
//# sourceMappingURL=ditaVersionDetector.js.map