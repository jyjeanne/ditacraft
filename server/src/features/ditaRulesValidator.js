...
exports.validateDitaRules = validateDitaRules;
...
const DITA_RULES = [
    // --- MANDATORY RULES (errors) ---
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<\\w+\\b(${patterns_1.TAG_ATTRS}\\brole\\s*=\\s*["']other["']${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (!/\botherrole\s*=/.test(match[1])) {
                    ...
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch001.missingOtherrole'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-001'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<(\\w+)\\b(${patterns_1.TAG_ATTRS}\\btype\\s*=\\s*["']other["']${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (!isNoteElement(match[1], match[2]))
                    continue;
                if (!/\bothertype\s*=/.test(match[2])) {
                    const attr = findAttrInTag(match[0], 'type');
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch002.missingOthertype'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-002'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch003.deprecatedIndextermref'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-003'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<(reltable|relcolspec)\\b(${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (/\bcollection-type\s*=/.test(match[2])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch004.collectionTypeNotAllowed', match[1]), node_1.DiagnosticSeverity.Error, 'DITA-SCH-004'));
                }
            }
        },
    },
    // --- RECOMMENDATION RULES (warnings) ---
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch010.deprecatedBoolean'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-010'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<image\\b(${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (/\balt\s*=/.test(match[1])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch011.deprecatedAltAttr'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-011'));
                }
            }
        },
    },
        ...
        check(text, diagnostics) {
            ...
            while ((match = regex.exec(text)) !== null) {
                if (/\blongdescref\s*=/.test(match[1])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch012.deprecatedLongdescref'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-012'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<(link|topicref)\\b(${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (/\bquery\s*=/.test(match[2])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch013.deprecatedQuery', match[1]), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-013'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<(\\w+)\\b(${patterns_1.TAG_ATTRS}\\bnavtitle\\s*=\\s*["'][^"']*["']${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (/^(?:topicref|chapter|appendix|part|keydef)$/.test(match[1])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch014.deprecatedNavtitle'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-014'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<map\\b(${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (/\btitle\s*=/.test(match[1])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch015.deprecatedMapTitle'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-015'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<shortdesc\\b${patterns_1.TAG_ATTRS}>([\\s\\S]*?)<\\/shortdesc>`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                const content = match[1].replace(new RegExp(`<(?:"[^"]*"|'[^']*'|[^>"'])+>`, 'g'), ''); // strip inner tags
                const wordCount = content.trim().split(/\s+/).filter(w => w.length > 0).length;
                if (wordCount > 50) {
                    diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch016.longShortdesc', wordCount), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-016'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<topichead\\b(${patterns_1.TAG_ATTRS})(?:\\/>|>([\\s\\S]*?)<\\/topichead>)`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                const attrs = match[1];
                ...
                const hasNavtitleElement = /<navtitle\b/.test(content)
                    || new RegExp(`<topicmeta\\b${patterns_1.TAG_ATTRS}>[\\s\\S]*?<navtitle\\b`).test(content);
                if (!hasNavtitleAttr && !hasNavtitleElement) {
                    diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch017.topicheadMissingNavtitle'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-017'));
                }
            }
        },
    },
    // --- AUTHORING RULES (warnings) ---
    {
        ...
        check(text, diagnostics) {
            // <xref> inside <title>
            const regex = new RegExp(`<title\\b${patterns_1.TAG_ATTRS}>([\\s\\S]*?)<\\/title>`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                const titleContent = match[1];
                if (/<xref\b/.test(titleContent)) {
                    const xrefPos = match.index + match[0].indexOf('<xref');
                    diagnostics.push(makeDiag(text, xrefPos, 5, (0, i18n_1.t)('sch020.xrefInTitle'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-020'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch021.requiredCleanup'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-021'));
            }
        },
    },
    {
            ...
            for (const { char, name } of tmChars) {
                ...
                while (pos !== -1) {
                    if (!isInsideTag(text, pos)) {
                        diagnostics.push(makeDiag(text, pos, 1, (0, i18n_1.t)('sch022.trademarkChar', name), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-022'));
                    }
    ...
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const sectionRegex = new RegExp(`<section\\b${patterns_1.TAG_ATTRS}>([\\s\\S]*?)<\\/section>`, 'g');
            let match;
            while ((match = sectionRegex.exec(text)) !== null) {
                const titleCount = countDirectChildTitles(match[1]);
                if (titleCount > 1) {
                    diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch023.multipleSectionTitles', titleCount), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-023'));
                }
            }
        },
    },
    // --- ACCESSIBILITY RULES (warnings) ---
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<image\\b(${patterns_1.TAG_ATTRS})(?:\\/>|>([\\s\\S]*?)<\\/image>)`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                const attrs = match[1];
                const content = match[2] || '';
                ...
                if (!hasAltAttr && !hasAltElement) {
                    diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch030.imageMissingAlt'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-030'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<object\\b${patterns_1.TAG_ATTRS}>([\\s\\S]*?)<\\/object>`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (!/<desc\b/.test(match[1])) {
                    diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch031.objectMissingDesc'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-031'));
                }
            }
        },
    },
    // --- NEW RULES (from dita-language-server's dita.sch) ---
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<xref\\b${patterns_1.TAG_ATTRS}(?<!\\/)>([\\s\\S]*?)<\\/xref>`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                const inner = match[1];
                ...
                if (nestedXref) {
                    ...
                    diagnostics.push(makeDiag(text, contentStart + nestedXref.index, 5, (0, i18n_1.t)('sch040.nestedXref'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-040'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<pre\\b${patterns_1.TAG_ATTRS}>([\\s\\S]*?)<\\/pre>`, 'g');
            ...
            while ((match = regex.exec(text)) !== null) {
                ...
                while ((fMatch = forbidden.exec(inner)) !== null) {
                    diagnostics.push(makeDiag(text, contentStart + fMatch.index, fMatch[0].length, (0, i18n_1.t)('sch041.forbiddenInPre', fMatch[1]), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-041'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<abstract\\b${patterns_1.TAG_ATTRS}>([\\s\\S]*?)<\\/abstract>`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (!/<shortdesc\b/.test(match[1])) {
                    diagnostics.push(makeDiag(text, match.index, '<abstract'.length, (0, i18n_1.t)('sch042.abstractMissingShortdesc'), node_1.DiagnosticSeverity.Information, 'DITA-SCH-042'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch043.noTopicNesting'), node_1.DiagnosticSeverity.Information, 'DITA-SCH-043'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch044.deprecatedRoleValue', match[1]), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-044'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const bodyRegex = new RegExp(`<(?:body|conbody)\\b${patterns_1.TAG_ATTRS}>([\\s\\S]*?)<\\/(?:body|conbody)>`, 'g');
            let match;
            while ((match = bodyRegex.exec(text)) !== null) {
                ...
                if (pCount === 1 && !otherBlocks) {
                    ...
                    diagnostics.push(makeDiag(text, match.index, tagLen, (0, i18n_1.t)('sch045.singleParagraphBody'), node_1.DiagnosticSeverity.Information, 'DITA-SCH-045'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<(section|example|fig|table|simpletable)\\b(${patterns_1.TAG_ATTRS})>`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                const attrs = match[2];
                if (/\bid\s*=/.test(attrs))
                ...
                if (/<title\b/.test(inner)) {
                    diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch046.idlessTitledElement', match[1]), node_1.DiagnosticSeverity.Information, 'DITA-SCH-046'));
                }
            }
        },
    },
    // --- DITA 2.0-SPECIFIC RULES ---
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch050.removedBoolean'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-050'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch051.removedIndextermref'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-051'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch052.removedObject'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-052'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            let match;
            while ((match = regex.exec(text)) !== null) {
                diagnostics.push(makeDiag(text, match.index, match[0].length, (0, i18n_1.t)('sch053.removedLearning', match[1]), node_1.DiagnosticSeverity.Error, 'DITA-SCH-053'));
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<audio\\b${patterns_1.TAG_ATTRS}(?:/>|>([\\s\\S]*?)<\\/audio>)`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (!match[1] || !/<fallback\b/.test(match[1])) {
                    diagnostics.push(makeDiag(text, match.index, '<audio'.length, (0, i18n_1.t)('sch054.audioMissingFallback'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-054'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<video\\b${patterns_1.TAG_ATTRS}(?:/>|>([\\s\\S]*?)<\\/video>)`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (!match[1] || !/<fallback\b/.test(match[1])) {
                    diagnostics.push(makeDiag(text, match.index, '<video'.length, (0, i18n_1.t)('sch055.videoMissingFallback'), node_1.DiagnosticSeverity.Warning, 'DITA-SCH-055'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<(\\w+)\\b(${patterns_1.TAG_ATTRS})`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                if (/\bprint\s*=/.test(match[2])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch056.removedPrint'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-056'));
                }
            }
        },
    },
        ...
        check(text, diagnostics) {
            ...
            while ((match = regex.exec(text)) !== null) {
                if (/\bcopy-to\s*=/.test(match[2])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch057.removedCopyTo'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-057'));
                }
            }
        },
    },
        ...
        check(text, diagnostics) {
            ...
            while ((match = regex.exec(text)) !== null) {
                if (/^(?:topicref|chapter|appendix|part|keydef|topichead)$/.test(match[1])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch058.removedNavtitle'), node_1.DiagnosticSeverity.Error, 'DITA-SCH-058'));
                }
            }
        },
    },
        ...
        check(text, diagnostics) {
            ...
            while ((match = regex.exec(text)) !== null) {
                if (/\bquery\s*=/.test(match[2])) {
                    ...
                    const offset = attr ? match.index + attr.offset : match.index;
                    const length = attr ? attr.length : match[0].length;
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('sch059.removedQuery', match[1]), node_1.DiagnosticSeverity.Error, 'DITA-SCH-059'));
                }
            }
        },
    },
    // --- ATTRIBUTE VALUE RULES ---
    {
        ...
        check(text, diagnostics) {
            ...
            const regex = new RegExp(`<note\\b(${patterns_1.TAG_ATTRS})>`, 'g');
            let match;
            while ((match = regex.exec(text)) !== null) {
                ...
                if (typeAttr && !noteTypeValues.has(typeAttr[1])) {
                    ...
                    diagnostics.push(makeDiag(text, offset, length, (0, i18n_1.t)('attr.invalidValue', typeAttr[1], 'type', [...noteTypeValues].join(', ')), node_1.DiagnosticSeverity.Warning, 'DITA-ATTR-001'));
                }
            }
        },
    },
        ...
        check(text, diagnostics) {
            ...
            while ((match = regex.exec(text)) !== null) {
                if (!validValues.has(match[1])) {
                    ...
                    diagnostics.push(makeDiag(text, offset, match[0].length, (0, i18n_1.t)('attr.invalidValue', match[1], 'importance', [...validValues].join(', ')), node_1.DiagnosticSeverity.Warning, 'DITA-ATTR-002'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            while ((match = regex.exec(text)) !== null) {
                if (!validValues.has(match[1])) {
                    const offset = match.index;
                    diagnostics.push(makeDiag(text, offset, match[0].length, (0, i18n_1.t)('attr.invalidValue', match[1], 'status', [...validValues].join(', ')), node_1.DiagnosticSeverity.Warning, 'DITA-ATTR-003'));
                }
            }
        },
    },
    {
        ...
        check(text, diagnostics) {
            ...
            while ((match = regex.exec(text)) !== null) {
                if (!validValues.has(match[1])) {
                    const offset = match.index;
                    diagnostics.push(makeDiag(text, offset, match[0].length, (0, i18n_1.t)('attr.invalidValue', match[1], 'translate', [...validValues].join(', ')), node_1.DiagnosticSeverity.Warning, 'DITA-ATTR-004'));
                }
            }
        },
    },
    // --- TABLE STRUCTURE RULES ---
    {
        ...
        check(text, diagnostics) {
            ...
            while ((tgroupMatch = tgroupRegex.exec(text)) !== null) {
                ...
                if (isNaN(declaredCols) || declaredCols <= 0)
                    continue;
                ...
                while ((rowMatch = rowRegex.exec(tgroupContent)) !== null) {
                    ...
                    if (entryCount > declaredCols) {
                        ...
                        diagnostics.push(makeDiag(text, rowOffset, rowMatch[0].length, (0, i18n_1.t)('table.colsMismatch', String(entryCount), String(declaredCols)), node_1.DiagnosticSeverity.Warning, 'DITA-TABLE-001'));
                    }
                }
            }
        },
            ...
            while ((tgroupMatch = tgroupRegex.exec(text)) !== null) {
                ...
                while ((spanMatch = spanAttrRegex.exec(tgroupContent)) !== null) {
                    ...
                    if (!colNames.has(refName)) {
                        ...
                        diagnostics.push(makeDiag(text, offset, spanMatch[0].length, (0, i18n_1.t)('table.invalidColRef', spanMatch[1], refName), node_1.DiagnosticSeverity.Warning, 'DITA-TABLE-002'));
                    }
                }
            }
        },
            ...
            while ((stMatch = stableRegex.exec(text)) !== null) {
                ...
                while ((rowMatch = rowRegex.exec(stContent)) !== null) {
                    ...
                    if (expectedCount === null) {
                        ...
                        diagnostics.push(makeDiag(text, rowOffset, rowMatch[0].length, (0, i18n_1.t)('table.stentryMismatch', String(count), String(expectedCount)), node_1.DiagnosticSeverity.Warning, 'DITA-TABLE-003'));
                    }
                }
            }
        },
        ...
        check(text, diagnostics) {
            ...
            while ((tgroupMatch = tgroupRegex.exec(text)) !== null) {
                if (!/<row\b/.test(tgroupMatch[1])) {
                    diagnostics.push(makeDiag(text, tgroupMatch.index, '<tgroup'.length, (0, i18n_1.t)('table.emptyTable'), node_1.DiagnosticSeverity.Information, 'DITA-TABLE-004'));
                }
            }
            ...
            while ((stMatch = stableRegex.exec(text)) !== null) {
                if (!/<strow\b/.test(stMatch[1])) {
                    diagnostics.push(makeDiag(text, stMatch.index, '<simpletable'.length, (0, i18n_1.t)('table.emptyTable'), node_1.DiagnosticSeverity.Information, 'DITA-TABLE-004'));
                }
            }
        },
    },
 ...
 */
function validateDitaRules(text, settings = DEFAULT_SETTINGS) {
    if (!settings.enabled)
        return [];
    ...
    for (const rule of DITA_RULES) {
        if (!rule.versions.includes(version))
            continue;
        if (!settings.categories.includes(rule.category))
            continue;
...
//  Helper functions
// ============================
function makeDiag(text, offset, length, message, severity, code) {
    return {
        range: (0, textUtils_1.offsetToRange)(text, offset, offset + length),
        message,
        severity,
        code,
        source: SOURCE,
 ...
 */
function countDirectChildTitles(content) {
    let count = 0;
    let depth = 0;
...
function isNoteElement(tagName, attrs) {
    if (/\bclass\s*=\s*["'][^"']*\btopic\/note\b/.test(attrs))
        return true;
    return tagName === 'note';
}
/**
 ...
 */
function findAttrInTag(tagText, attrName) {
...
//# sourceMappingURL=ditaRulesValidator.js.map