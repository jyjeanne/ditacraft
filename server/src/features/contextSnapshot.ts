...
import { GetContextGraphParams, handleGetContextGraph, ContextGraph, MapNode, TopicRefNode, TopicNode } from './contextGraph';
import { IKeySpaceService } from '../services/interfaces';

// ── Request / Response types (mirrored on the client) ─────────────────────

export interface BuildContextSnapshotParams {
    uri: string;
    maxTokens: number;
    strategy: 'breadth-first' | 'depth-first' | 'by-relevance';
    /** Optional URI of the topic nearest to the cursor — used for Level 3 sliding window. */
    focusUri?: string;
}
...
function renderMapNodeXml(
    node: MapNode | TopicRefNode,
...
export function handleBuildContextSnapshot(
    params: BuildContextSnapshotParams,
    keySpaceService?: IKeySpaceService
): ContextSnapshotResult {
    const graphParams: GetContextGraphParams = {
        uri: params.uri,
        depth: 4,
        includeMetadata: true,
    };

    const graph = handleGetContextGraph(graphParams, keySpaceService);
