...
exports.validateContentModel = validateContentModel;
const node_1 = require("vscode-languageserver/node");
const DITA_CONTENT_MODELS = {
    // Map elements
    'map': {
        allowedChildren: [
    ...
    while ((match = tagPattern.exec(cleaned)) !== null) {
        ...
        if (isClosing) {
            if (stack.length > 0) {
                ...
                if (top.name !== tagName) {
                    const range = node_1.Range.create(pos.line, pos.column, pos.line, pos.column + fullMatch.length);
                    parseDiags.push(node_1.Diagnostic.create(range, `Mismatched closing tag: expected </${top.name}>, found </${tagName}>`, node_1.DiagnosticSeverity.Error, 'DITA-CM-PARSE', 'dita-lsp'));
                }
                stack.pop();
            }
        }
        else {
            const element = { name: tagName, children: [], line: pos.line, column: pos.column };
            if (stack.length > 0) {
...
// Validation logic
// ---------------------------------------------------------------------------
function validateElement(element, diagnostics) {
        ...
        if (model.disallowedChildren) {
            for (const child of element.children) {
                if (model.disallowedChildren.includes(child.name)) {
                    const range = node_1.Range.create(child.line, child.column, child.line, child.column + child.name.length + 2);
                    diagnostics.push(node_1.Diagnostic.create(range, `Invalid element <${child.name}> inside <${element.name}>. ${model.description || ''}`, node_1.DiagnosticSeverity.Error, 'DITA-CM-001', 'dita-lsp'));
                }
            }
        }
        ...
        if (model.requiredChildren) {
            for (const required of model.requiredChildren) {
                if (!element.children.some(c => c.name === required)) {
                    const range = node_1.Range.create(element.line, element.column, element.line, element.column + element.name.length + 2);
                    diagnostics.push(node_1.Diagnostic.create(range, `Missing required child element <${required}> in <${element.name}>`, node_1.DiagnosticSeverity.Error, 'DITA-CM-002', 'dita-lsp'));
                }
            }
        }
        ...
        if (model.allowedChildren) {
            for (const child of element.children) {
                if (!model.allowedChildren.includes(child.name) &&
                    ...
                    const range = node_1.Range.create(child.line, child.column, child.line, child.column + child.name.length + 2);
                    diagnostics.push(node_1.Diagnostic.create(range, `Element <${child.name}> may not be valid inside <${element.name}>`, node_1.DiagnosticSeverity.Warning, 'DITA-CM-003', 'dita-lsp'));
                }
            }
        }
    }
 ...
 */
function validateContentModel(text) {
    const { root, parseDiags } = parseElementTree(text);
    const diagnostics = [...parseDiags];
    if (root) {
        validateElement(root, diagnostics);
    }
    return diagnostics;
}
//# sourceMappingURL=contentModelValidation.js.map