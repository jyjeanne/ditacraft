 ...
 */
function applySuppressions(diagnostics, text) {
    ...
    return diagnostics.filter(d => {
        ...
        if (state.fileSuppressed.has(code))
            return false;
        ...
        for (const r of state.ranges) {
            if (r.code === code && line >= r.startLine && line < r.endLine)
                return false;
        }
        return true;
    });
}
//# sourceMappingURL=suppressionEngine.js.map