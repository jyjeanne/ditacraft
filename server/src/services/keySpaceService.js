...
exports.KeySpaceService = void 0;
exports.reportKeySpace = reportKeySpace;
exports.formatResolutionReport = formatResolutionReport;
...
const MAX_KEY_SPACE_ENTRIES = 50000;
...
class KeySpaceService {
    constructor(workspaceFolders, getSettings, log) {
     ...
     */
    setExplicitRootMap(rootMapPath) {
     ...
     */
    async resolveKey(keyName, contextFilePath) {
     ...
     */
    async explainKey(keyName, contextFilePath) {
        const report = {
            keyName,
            contextFilePath,
     ...
     */
    async getAllKeys(contextFilePath) {
        const rootMap = await this.findRootMap(contextFilePath);
        if (!rootMap) {
            return new Map();
        }
     ...
     */
    async getSubjectSchemePaths(contextFilePath) {
        const rootMap = await this.findRootMap(contextFilePath);
        if (!rootMap) {
            return [];
        }
     ...
     */
    async getDuplicateKeys(contextFilePath) {
        const rootMap = await this.findRootMap(contextFilePath);
        if (!rootMap) {
            return new Map();
        }
     ...
     */
    async buildKeySpace(rootMapPath) {
    ...
    async findRootMap(filePath) {
        ...
        while (currentDir && currentDir.length >= stopDir.length) {
            try {
                ...
                const mapFiles = files.filter(f => f.endsWith('.ditamap') || f.endsWith('.bookmap'));
    ...
    async doBuildKeySpace(absoluteRootPath) {
        ...
        while (queue.length > 0) {
            ...
            try {
                const rawContent = await fs_1.promises.readFile(currentMap, 'utf-8');
                ...
                const mapContent = this.stripReltables((0, textUtils_1.stripCommentsAndCDATA)(rawContent));
                ...
                const maskedContent = this.processInlineScopeBlocks(mapContent, currentMap, effectivePrefixes, keySpace, scopeDirectKeys, queue, maxLinkMatches);
                ...
                this.extractTopicReferences(maskedContent, currentMap, primaryPrefix, keySpace.topicToScope, maxLinkMatches);
     ...
     */
    parseMapElements(mapContent) {
        // Strip DOCTYPE — fast-xml-parser chokes on internal subsets.
        const stripped = mapContent.replace(/<!DOCTYPE\s[\s\S]*?(?:\[[\s\S]*?\]\s*)?>|<!DOCTYPE[^>]*>/gi, '');
        try {
            const parsed = this.xmlParser.parse(stripped);
            const results = [];
     ...
     */
    collectXmlElements(obj, out, tagName = '') {
        if (typeof obj !== 'object' || obj === null)
            return;
        if (Array.isArray(obj)) {
            for (const item of obj)
                this.collectXmlElements(item, out, tagName);
            return;
        }
        const node = obj;
        if (tagName) {
     ...
     */
    extractMetadataFromNode(node) {
        const rawTopicmeta = node['topicmeta'];
        if (!rawTopicmeta || typeof rawTopicmeta !== 'object')
        ...
        if (typeof navtitleRaw === 'string') {
            ...
            const t = navtitleRaw['#text'];
            if (typeof t === 'string')
                metadata.navtitle = t.trim() || undefined;
        }
        // keywords / keyword
        const kwBlock = meta['keywords'];
        if (kwBlock && typeof kwBlock === 'object') {
            const kwRaw = kwBlock['keyword'];
            const kwList = [];
            const addKw = (v) => {
                if (typeof v === 'string')
                    kwList.push(v.trim());
            ...
            if (Array.isArray(kwRaw))
                ...
                addKw(kwRaw);
            if (kwList.length > 0) {
                metadata.keywords = kwList;
                inlineContent = kwList[0];
            }
        }
        if (!inlineContent) {
            const directKw = meta['keyword'];
            if (typeof directKw === 'string') {
                inlineContent = directKw.trim();
        ...
        if (typeof sdRaw === 'string')
            metadata.shortdesc = sdRaw.trim() || undefined;
     ...
     */
    extractKeyDefinitions(mapContent, mapPath, maxMatches) {
    ...
    extractKeyDefinitionsFromElements(elements, mapPath, maxMatches, mapContent) {
        const keys = [];
        ...
        for (const { attrs, node } of elements) {
            ...
            if (attrs['keyscope'] && /\.(?:ditamap|bookmap)$/i.test(attrs['href'] ?? ''))
                continue;
            ...
            for (const keyName of keyNames) {
                const keyDef = { keyName, sourceMap: mapPath };
                if (sourceLine !== undefined)
                    keyDef.sourceLine = sourceLine;
                    ...
                    if (href.includes('#')) {
                        ...
                        if (filePart) {
                            ...
                            if (this.isPathWithinWorkspace(resolved))
                                keyDef.targetFile = resolved;
                        }
                        if (elemId)
                            keyDef.elementId = elemId;
                    }
                    else {
                ...
                if (attrs['scope'])
                    keyDef.scope = attrs['scope'];
                if (attrs['processing-role'])
                    keyDef.processingRole = attrs['processing-role'];
                if (attrs['keyref'])
                    keyDef.keyref = attrs['keyref'];
                if (!isSelfClosing) {
                    const metaResult = this.extractMetadataFromNode(node);
                    if (metaResult.metadata)
                        keyDef.metadata = metaResult.metadata;
    ...
    extractKeyDefinitionsRegex(mapContent, mapPath, maxMatches) {
        const keys = [];
        const mapDir = path.dirname(mapPath);
        const keydefRegex = new RegExp(`<(\\w+)\\b${patterns_1.TAG_ATTRS}\\bkeys\\s*=\\s*["']([^"']+)["']${patterns_1.TAG_ATTRS}>`, 'gi');
        let match;
        let matchCount = 0;
     ...
     */
    extractKeyMetadata(mapContent, startIndex) {
        ...
        const topicmetaMatch = afterElement.match(/<topicmeta\b[^>]*>([\s\S]*?)<\/topicmeta>/i);
        if (!topicmetaMatch) {
            return { metadata: null, inlineContent: null };
        }
        ...
        const keywordsBlockMatch = metaContent.match(/<keywords\b[^>]*>([\s\S]*?)<\/keywords>/i);
     ...
     */
    followKeyrefChain(keyDef, keys, scopePrefix = '', hopsRemaining = 3, visited = new Set()) {
     ...
     */
    followKeyrefChainWithTrace(startDef, keys, scopePrefix) {
        if (!startDef.keyref)
            return { def: startDef, chain: [] };
        const chain = [];
    ...
    appendKeyrefSteps(steps, chain, keys) {
     ...
     */
    extractTopicReferences(mapContent, mapPath, scopePrefix, topicToScope, maxMatches) {
        const mapDir = path.dirname(mapPath);
        const topicRefRegex = new RegExp(`<(\\w+)\\b${patterns_1.TAG_ATTRS}\\bhref\\s*=\\s*["']([^"'#]+\\.(?:dita|xml))(?:#[^"']*)?["']`, 'gi');
        let match;
        let count = 0;
        while ((match = topicRefRegex.exec(mapContent)) !== null) {
            if (++count > maxMatches)
                break;
            if (['mapref', 'keydef', 'subjectdef'].includes(match[1].toLowerCase()))
                continue;
            const href = match[2];
    ...
    extractMapReferences(mapContent, mapPath, maxLinkMatches) {
        const submaps = [];
        const mapDir = path.dirname(mapPath);
        ...
        const mapRefRegex = new RegExp(`<\\w+\\b${patterns_1.TAG_ATTRS}\\bhref\\s*=\\s*["']([^"']+\\.(?:ditamap|bookmap))["']${patterns_1.TAG_ATTRS}>`, 'gi');
        let match;
        let matchCount = 0;
    ...
    cleanupExpiredRootMapCache() {
        ...
        for (const key of expired)
            this.rootMapCache.delete(key);
    }
    startPeriodicCleanup() {
        const interval = Math.max(MIN_CLEANUP_INTERVAL_MS, this.cacheConfig.ttlMs / CACHE_CLEANUP_RATIO);
    ...
    isPathWithinWorkspace(absolutePath) {
     ...
     */
    combineScopePrefixes(parentPrefixes, childScopes) {
     ...
     */
    addScopedKeyEntry(keySpace, qualifiedName, def) {
    ...
    computeLineNumber(content, charIndex) {
        let line = 1;
     ...
     */
    extractInlineScopeBlocks(content) {
        const blocks = [];
        const keyscopedRe = new RegExp(`<(\\w+)\\b${patterns_1.TAG_ATTRS}\\bkeyscope\\s*=\\s*["']([^"']+)["']${patterns_1.TAG_ATTRS}>`, 'gi');
        let match;
        while ((match = keyscopedRe.exec(content)) !== null) {
            ...
            if (/^(?:map|bookmap)$/i.test(elemName))
                continue;
            // Submap references are handled by extractMapReferences + inlineKeys mechanism
            if (/\bhref\s*=\s*["'][^"']*\.(?:ditamap|bookmap)["']/i.test(fullOpenTag))
                continue;
     ...
     */
    findInnerContent(content, fromIndex, tagName) {
        ...
        while (depth > 0 && pos < content.length) {
            ...
            if (nextOpen && nextOpen.index < nextClose.index) {
                ...
                while (checkPos > nextOpen.index && /\s/.test(content[checkPos]))
                    checkPos--;
                if (content[checkPos] !== '/')
                    depth++;
                pos = scanPos + 1;
    ...
    maskRanges(content, ranges) {
        ...
        for (const { start, end } of sorted) {
            if (start > pos)
                result += content.substring(pos, start);
     ...
     */
    processInlineScopeBlocks(content, mapPath, parentEffectivePrefixes, keySpace, scopeDirectKeys, bfsQueue, maxLinkMatches, depth = 0) {
        if (depth > MAX_INLINE_SCOPE_DEPTH)
            return content;
        ...
        for (const block of blocks) {
            ...
            for (const inlineKeyName of block.inlineKeys) {
                const inlineDef = { keyName: inlineKeyName, sourceMap: mapPath };
                for (const prefix of childPrefixes) {
                    const directKeys = scopeDirectKeys.get(prefix);
                    if (!directKeys.some(k => k.keyName === inlineKeyName))
                        directKeys.push(inlineDef);
            ...
            const maskedBlockContent = this.processInlineScopeBlocks(block.innerContent, mapPath, childPrefixes, keySpace, scopeDirectKeys, bfsQueue, maxLinkMatches, depth + 1);
            ...
            this.extractTopicReferences(maskedBlockContent, mapPath, childPrefixes[0], keySpace.topicToScope, maxLinkMatches);
     ...
     */
    stripReltables(content) {
        return content.replace(/<reltable\b[^>]*>[\s\S]*?<\/reltable\s*>/gi, '');
    }
    /** Extract keyscope(s) from the root map/bookmap element. */
    extractRootKeyscope(mapContent) {
        const rootMatch = mapContent.match(new RegExp(`<(?:map|bookmap)\\b(${patterns_1.TAG_ATTRS})`, 'i'));
        if (!rootMatch)
            return [];
    ...
    async fileExistsAsync(filePath) {
        try {
            await fs_1.promises.access(filePath, fs.constants.F_OK);
            return true;
        }
        catch {
            return false;
...
exports.KeySpaceService = KeySpaceService;
// ---------------------------------------------------------------------------
 ...
 */
function reportKeySpace(keySpace) {
    const lines = [];
 ...
 */
function formatResolutionReport(report) {
    const lines = [];
...
//# sourceMappingURL=keySpaceService.js.map